...
| HTML startup response | Supplied a RunPod-style `<!DOCTYPE html>` startup page. The Android policy retries it once, rejects it after the bounded retry, and uses friendly startup wording. The backend rejects HTML provider output and returns its controlled fallback. | **Pass** | `ResilientRequestPolicyTest`, `BitwiseBackendClientTest`, and `bitwiseGemini.test.js` |
| HTTP 502, 503, and 504 | Supplied each transient gateway status. Every status receives no more than one retry and resolves to the same safe shopper message without exposing the raw provider body. | **Pass** | `BitwiseBackendClientTest.gatewayFailuresUseTheSameFriendlyBoundedFallback` |
